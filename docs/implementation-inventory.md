...
| `sim/controller.py` | Comando nominal, PID, transições e anti-windup | change `digital-servo-foundations` | `tests/test_components.py` |
...
| `sim/metrics.py` | Erro RMS percentual após estabilização | change `digital-servo-foundations` | `tests/test_metrics.py` |
...
| Cinco gráficos com janela de 20 segundos | change `digital-servo-foundations` | validação visual manual |
...
| Comando nominal e PID ajustável | change `digital-servo-foundations` | testes de PID em `test_components.py` e `test_simulator.py` |
...
| Encoder discreto, filtro e falhas | change `digital-servo-foundations` | testes de encoder em `test_components.py` |
